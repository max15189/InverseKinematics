...
Workflow:
  1. generate_dataset()  — sample joint configs, compute FK, save raw .npy files
  2. split_and_save()    — split into train / val / test and save per-split files
  3. load_split()        — load a single split back from disk
"""
...
def validate_dataset(
    ...
    """
    Spot-check: verify FK(q1 + dq) ≈ x_d for random samples.
    Prints mean and max position error (should be ~0).
    """
...
def save_raw(save_dir: str, q1: np.ndarray, xd: np.ndarray, dq: np.ndarray) -> None:
    """Save raw arrays to save_dir as q1.npy, xd.npy, dq.npy."""
...
def load_raw(save_dir: str) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    """Load raw q1 / xd / dq arrays from save_dir."""
...
def split_and_save(
    ...
    per-split files (q1_train.npy, xd_val.npy, …) to save_dir.

    Args:
        test_size:     fraction of data held out for val+test combined.
        val_fraction:  fraction of the held-out set used for validation
                       (the rest becomes test).  Default 0.5 → 10/10 split.
    """
    q1, xd, dq = load_raw(save_dir)
...
def load_split(save_dir: str, split: str) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    """
    Load one split (train / val / test) from save_dir.

    Returns:
        q1, xd, dq as float32 arrays.
    """
...
if __name__ == "__main__":
    ...
    q1, xd, dq = generate_dataset(n_base=50_000, n_neighbours=5, delta=0.05)

    print(f"\nDataset size: {len(q1):,} samples")
    validate_dataset(q1, xd, dq)

    save_raw(SAVE_DIR, q1, xd, dq)
